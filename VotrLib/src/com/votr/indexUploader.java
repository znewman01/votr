...
public class indexUploader {
	...
	public void loadTable() {
		...
		for (Map<String, AttributeValue> item : result.getItems()) {
			Vote v = new Vote(item.get("voter_id").getS(), item.get("poll_id")
					.getS(), (item.get("choice") == null) ? null : item.get(
					"choice").getN(), (item.get("city") == null) ? null : item
					.get("city").getS(), (item.get("state") == null) ? null
					: item.get("state").getS(),
					(item.get("zipcode") == null) ? null : item.get("zipcode")
							.getS(), (item.get("tags") == null) ? null : item
							.get("tags").getSS());
			votes.add(v);
	...
	public void indexVote(List<Vote> votes, int version) {
		...
		HttpPost post = new HttpPost(
				"http://doc-test-xnrvb2xw2rc2iq76porynyunqy.us-east-1.cloudsearch.amazonaws.com");
		post.addHeader(new BasicHeader("Content-Type", "application/json"));
		post.addHeader(new BasicHeader("Host",
				"doc-test-xnrvb2xw2rc2iq76porynyunqy.us-east-1.cloudsearch.amazonaws.com"));
		post.addHeader(new BasicHeader("Accept", "application/json"));

		try {
			post.setURI(new URI(
					"http://doc-test-xnrvb2xw2rc2iq76porynyunqy.us-east-1.cloudsearch.amazonaws.com/2011-02-01/documents/batch"));

			ObjectMapper mapper = new ObjectMapper();
			List<Object> docs = new ArrayList<Object>();

			for (Vote vote : votes) {
				String doc_id = (new Integer(Math.abs(vote.voter_id.hashCode())))
						.toString()
						+ (new Integer(Math.abs(vote.poll_id.hashCode())))
								.toString();
				...
				fields.put("choice_count", (vote.choice == null) ? ""
						: vote.choice);
				...
				fields.put("state_count", (vote.state == null) ? ""
						: vote.state);
				fields.put("zipcode", (vote.zipcode == null) ? ""
						: vote.zipcode);
